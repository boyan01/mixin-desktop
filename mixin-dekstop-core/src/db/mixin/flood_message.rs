...
impl MixinDatabase {
    pub async fn insert_flood_message(&self, message: FloodMessage) -> Result<(), Error> {
        let _ = sqlx::query("INSERT INTO flood_messages (message_id, data, created_at) VALUES (?, ?, ?)")
    ...
    pub async fn flood_messages(&self) -> Result<Vec<FloodMessage>, Error> {
        let result = sqlx::query_as::<_, FloodMessage>("SELECT * FROM flood_messages ORDER BY created_at ASC LIMIT 10")
    ...
    pub async fn latest_flood_message_created_at(&self) -> Result<Option<NaiveDateTime>, Error> {
        let latest = sqlx::query_scalar::<_, NaiveDateTime>("SELECT created_at FROM flood_messages ORDER BY created_at DESC LIMIT 1")
