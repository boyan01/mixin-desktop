...
- `desktop/` is the macOS product target. Build new desktop UI with SwiftUI and keep the reusable Rust core.
...
- Maintain `desktop/PARITY.md` as the living feature inventory once implementation starts. Each entry records the source entry path, required Rust API, Swift status, validation evidence, and a concrete unresolved question when blocked.
...
generated Swift UniFFI API
      |
      v
desktop/rust (Swift-only lowering and DTO conversion)
      |
      v
...
- `desktop/rust` depends on `mixin_desktop_api`, never directly on `mixin_desktop_core`. It contains UniFFI annotations, bridge-safe DTO conversion, typed errors, and subscription lowering only.
...
- Keep generated Swift in `desktop/desktop/Generated/` and generated C/module-map files in `desktop/generated/`. Never hand-edit them.
...
- Generated bindings are committed. Regenerate Swift source, C header, and module map with the workflow in `desktop/README.md` after every UniFFI-visible change.
...
- Use Swift Testing or XCTest model tests under `desktopTests/`. Use `desktopUITests/` for a small set of high-value navigation, keyboard, menu, drag/drop, and accessibility flows.
...
xcodebuild \
  -project desktop/desktop.xcodeproj \
  -scheme desktop \
  -configuration Debug \
  -derivedDataPath /tmp/mixin-desktop-swift-derived \
  CODE_SIGNING_ALLOWED=NO \
  build
```

- Run the relevant `xcodebuild test` destinations after test targets exist.
- For a UniFFI-visible change, regenerate all bindings using `desktop/README.md`, then run Rust tests, Swift model tests, and the Xcode Debug build.
