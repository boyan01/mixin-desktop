...
class ChatScrollCoordinator {
  ...
  void scheduleRestore({
    required List<MessageListEntry> messages,
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (!request.reset) _applyViewportAnchorCorrection();
      _restore(request);
  ...
  void _applyViewportAnchorCorrection() {
    final snapshot = _viewportAnchorSnapshot;
    _viewportAnchorSnapshot = null;
    if (snapshot == null || !scrollController.hasClients) return;
    final top = _messageTopInViewport(snapshot.messageId, _keysByMessageId);
    if (top == null) {
      traceChatJump(
  ...
  bool _jumpToMessage(
    ...
    unawaited(
      _jumpToClamped(
        ...
        _traceTargetAfterLayout('restore-after', messageId, keysByMessageId);
        onComplete?.call();
      }),
    );
    return true;
  ...
  _MessageTargetGeometry? _messageTargetGeometry(
    ...
    final renderOrigin = _originInViewport(render, viewport);
    if (renderOrigin == null) return null;

    final top = renderOrigin.dy;
  ...
  double? _messageTopInViewport(
    ...
    return renderOrigin.dy;
  }

  Offset? _originInViewport(RenderBox render, RenderBox viewport) {
    try {
      return render.localToGlobal(Offset.zero, ancestor: viewport);
    } catch (error) {
  ...
  Future<void> _jumpToClamped(
    ...
    return _runProgrammaticScroll(() {
      ...
      if (animationDirection != null) {
        _jumpToDirectionalAnimationStart(target, animationDirection, position);
        return scrollController.animateTo(
          target,
          duration: _jumpAnimationDuration,
          curve: _jumpAnimationCurve,
  ...
  void _setMessages(
    ...
    final previousBottomMessageId = _messages.isEmpty
        ? null
        : _messages.last.id;
    final nextBottomMessageId = messages.isEmpty ? null : messages.last.id;
    final messageWindowChanged = !_hasSameMessageIds(messages);
  ...
  bool _hasSameMessageIds(List<MessageListEntry> messages) {
    if (identical(_messages, messages)) return true;
    if (_messages.length != messages.length) return false;
    for (var i = 0; i < messages.length; i++) {
      if (_messages[i].id != messages[i].id) return false;
    }
    return true;
  }
  ...
  void _traceTargetAfterLayout(
    ...
    void trace(String step) {
      ...
      traceChatJump(
        ...
        'topDelta=${formatDouble(geometry == null ? null : geometry.top - focusY)} '
        'unreadY=${formatDouble(unreadY)} '
        'bottomDelta=${formatDouble(geometry == null ? null : geometry.bottom - unreadY)} '
        '${formatScrollMetrics(scrollController.position)}',
      );
    }
...
class _ViewportAnchorSnapshot {
  const _ViewportAnchorSnapshot({required this.messageId, required this.top});
